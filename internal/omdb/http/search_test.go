...
func TestSearchCreateRequest(t *testing.T) {
	c, err := NewHTTPOMDB("testendpoint", "secretkey")
	if err != nil {
		t.Fatal(err)
	}
	req, err := c.createSearchRequest(context.Background(), omdb.SearchRequest{
		Query: "some movie title",
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSearchParseResponse(t *testing.T) {
	response := `{
		"Search": [
			{
	...
	resp, err := parseSearchResponse([]byte(response))
	if err != nil {
		t.Fatal(err)
	}
	fmt.Println(resp)
